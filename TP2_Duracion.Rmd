...
Si bien con un nivel de significación del 5% el solo el modelo estratificado cumple el supuesto de hazards proporcionales (Tabla x), el p-value asociado al test sin estratificar es muy cercano a 0.05 y ese modelo nos da la posibilidad de estimar un parámetro para la Quimioterapia, que es una variable importante para el problema. Por lo tanto, decidimos quedarnos con ese modelo. De esta forma, el modelo final resulta:
...
Por lo que el modelo final estimado resulta ser $$\hat{h_i} (t) = \hat{h_0} (t) \cdot exp(-0.05 \cdot A_{1i} + 0.5 \cdot A_{2i} + 0.77 \cdot A_{3i} + 0.35 \cdot \text{chemo}_i + 0.31 \cdot G_{2i} + 0.38 \cdot S_{1i} + 0.84 \cdot S_{2i})$$
