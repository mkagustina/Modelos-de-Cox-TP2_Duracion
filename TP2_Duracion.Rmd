...
modelo_estrat <- coxph(Surv(dtime, death) ~ as.factor(age_factor) + 
                      strata(chemo) + grade + size, ties = "breslow",
                      data = datos_age_cat)
# test2 <- cox.zph(modelo_estrat)
cox.zph(modelo_estrat) # GLOBAL:0.087

# Modelo sin estratificar
modelo_sin_estrat <- coxph(Surv(dtime, death) ~ as.factor(age_factor) +
                             chemo + grade + size, ties = "breslow",
                       data = datos_age_cat)
