...
```{r}
# Modelos usando la mejor categorización para edad. Pruebo un modelo estratificando por chemo y otro sin estratificar
...
# Edad factor
modelo_estrat <- coxph(Surv(dtime, death) ~
                      as.factor(age_factor) + strata(chemo)+
                      grade + size, ties = "breslow",
                      data = datos2)
# test2 <- cox.zph(modelo_estrat)
...
modelo_sin_estrat <- coxph(Surv(dtime, death) ~
                             age_factor + chemo + grade + size,
                           ties = "breslow", data = datos2)
# test2 <- cox.zph(modelo_sin_estrat)
cox.zph(modelo_sin_estrat) # GLOBAL:0.01

# Edad factor
modelo_sin_estrat <- coxph(Surv(dtime, death) ~
                             as.factor(age_factor) + chemo +
                             grade + size, ties = "breslow",
                           data = datos2)
# test2 <- cox.zph(modelo_sin_estrat)
cox.zph(modelo_sin_estrat) # GLOBAL:0.046 (pero todas las variables, incluida chemo dan significativas)
```
