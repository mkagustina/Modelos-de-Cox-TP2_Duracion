...
Previo a esto, se definen 2 *dummies* referentes a la variable *Tamaño* y 1 para la variable *Grado*:
...
\vspace{-0.5cm}
\begin{center}
\footnotesize{\textit{Tabla 1: Resultados de test de hipótesis para modelos univariados}}
\end{center}
...
\vspace{-0.5cm}
\begin{center}
\footnotesize{\textit{Tabla 2: Resultados de test de hipótesis para eliminar variables}}
\end{center}
...
\vspace{-0.5cm}
\begin{center}
\footnotesize{\textit{Tabla 3: resultados de test de hipótesis para incluir variables}}
\end{center}
...
En este caso, la única variable continua que se tiene es *Edad*. Por lo que, las hipótesis que se plantean son:
...
Se comprabará mediante los residuos de Schoenfeld obtenidos para cada variable y el test de Grambsch y Therneau.
...
\vspace{-0.5cm}
\begin{center}
\footnotesize{\textit{Tabla 4: Resultados del test de Grambsch y Therneau}}
\end{center}
...
Los resultados del test (Tabla 4), de manera global, dicen que el supuesto no es cierto ya que se rechaza la hipótesis nula. Además, brindan información acerca de cuáles variables no estarían cumpliendo la proporcionalidad de los hazards: *Edad* y *Quimioterapia*. Esto puede constatarse también de forma gráfica (Gráfico 1)al ver que los residuos de *Quimioterapia* no están divididos en dos grupos (cantidad de categorías que tiene), lo que sí sucede con *Grado* y *Tamaño*. 
...
\vspace{-0.5cm}
\begin{center}
\footnotesize{\textit{Tabla 5: Resultados del test de Grambsch y Therneau}}
\end{center}

Si bien con un nivel de significación del 5% el modelo estratificado cumple el supuesto de hazards proporcionales (Tabla 5 -derecha-), la probabilidad asociada al test sin estratificar es muy cercano a 0.05 (Tabla 5 -izquierda-) y ese modelo brinda la posibilidad de estimar un parámetro para *Quimioterapia*, que es una variable importante para el problema. Por lo tanto, se decide  con ese modelo. De esta forma, el modelo final contiene las siguientes variables:
...
#### 1) Grado y Tamaño: 
Es sabido que un mayor grado de diferenciciación más rápida es la dispersión del cancer, por lo que se calcula la razón de hazard para el grado de diferenciación 3 y comparando, por un lado, el tamaño del tumor menor o igual a 20 mm contra el tamaño entre 20 y 50 mm; por otro, el tamaño del tumor menor o igual a 20 mm contra el tamaño mayor a 50 mm.

#### 2) Tamaño y Quimioterapia: 
Muchas veces, la quimioterapia no da resultados; esto no necesariamente se da porque se la haya aplicado tarde, si no porque, quizás, el tamaño del tumor a tratar era demasiado pequeño.

#### 3) Edad: 
Resulta de interés comparar las funciones hazards de mujeres que fueron expuestas a la quimioterapia, poseen grado 3 de diferenciación y su tumor es de más de 50mm, en especifico para dos grupos etarios: las menores de 40 años y aquellas entre 50 y 65 años.


$$ \begin{array} {c|cc}
\hat{RH} & \text{Estimación} & \text{Intervalo} \\
\hline
\\
G=3; \; T = \leq 20 \; \text{vs} \; 20\text{-}50 & 1.46 & (1.161; 1.842) \\
\\
G=3; \; T = \leq 20 \; \text{vs} \; >50 & 2.32 & (1.692; 3.171) \\
\\
Q=Si, \; T= \leq 20; \; \text{vs} \; Q=No, \; T= \; >50 & 1.63 & (1.034; 2.578) \\
\\
Q=No, \; T= \leq 20; \; \text{vs} \; Q=Si, \; T= \; >50 & 3.29 & (2.140;5.048) \\
\\
Q=Si; \; G=3; \; T=>50; \; E=<40 \; \text{vs} \; \geq50 \;\text{-}<65 & 1.65 & (1.092;2.488) \\
\end{array}$$

#### 1) 
La tasa de mortalidad de pacientes con grado 3 de diferenciación y un tumor entre 20 y 50 mm es, como minimo, un 16.1% mayor y, como máximo, un 84.2% mayor que esa misma tasa de pacientes con grado 3 de diferenciación y un tumor de 20 mm o menos. Mientras que la tasa de mortalidad de pacientes con grado 3 de diferenciación y un tumor de más de 50 mm es, como mínimo, un 69.2% mayor y, como máximo, un 217% mayor que esa misma tasa de pacientes también con grado 3 pero con un tumor de 20mm o menos; para valores fijos de las variables restantes.

#### 2) 
La tasa de mortalidad para mujeres que no recibieron quimioterapia teniendo un tumor de más de 50 mm es, como mínimo, un 3.4% mayor y, a lo sumo, un 158% mayor que esa misma tasa para mujeres que si recibieron quimioteapia y tenían un tumor de 20 mm o menos. 
Por otro lado, la tasa de mortalidad para mujeres que tenían un tumor de más de 50 mm y recibieron quimioterapia es, al menos, un 114% mayor y, como máximo, 405% mayor que esa misma tasa para mujeres cuyo tumor medía 20 mm o menos y no recibieron quimioterapia.

#### 3) 
La tasa de mortalidad de mujeres entre 50 y 65 años es, al menos, un 9.2% mayor y, como mucho, casi un 150% mayor que esa misma tasa para mujeres de menos de 40 años; para los valores establecidos anteriormente de las demás variables.
