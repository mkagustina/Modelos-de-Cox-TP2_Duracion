...
$$
h_i(t) = h_0(t) \cdot \exp(\beta_1 \cdot \text{age}_i + \beta_2 \cdot \text{meno}_i + \beta_3 \cdot \text{er}_i + \beta_4 \cdot G_{1i} + \beta_5 \cdot G_{2i} + \beta_6 \cdot S_{1i} + \beta_7 \cdot S_{2i})
$$
...
$$h_i(t) = h_0(t) \cdot \exp(\beta_1 \cdot \text{age}_i + \beta_3 \cdot \text{er}_i + \beta_4 \cdot G_{1i} + \beta_5 \cdot G_{2i} + \beta_6 \cdot S_{1i} + \beta_7 \cdot S_{2i})$$

3) Sin la variable *Receptores de estrógeno*: 

$$h_i(t) = h_0(t) \cdot \exp(\beta_1 \cdot \text{age}_i + \beta_2 \cdot \text{meno}_i + \beta_4 \cdot G_{1i} + \beta_5 \cdot G_{2i} + \beta_6 \cdot S_{1i} + \beta_7 \cdot S_{2i})$$

4) Sin la variable *Grado de diferenciación del tumor*: 

$$h_i(t) = h_0(t) \cdot \exp(\beta_1 \cdot \text{age}_i + \beta_2 \cdot \text{meno}_i + \beta_3 \cdot \text{er}_i + \beta_6 \cdot S_{1i} + \beta_7 \cdot S_{2i})$$

5) Sin la variable *Tamaño del tumor*: 

$$h_i(t) = h_0(t) \cdot \exp(\beta_1 \cdot \text{age}_i + \beta_2 \cdot \text{meno}_i + \beta_3 \cdot \text{er}_i + \beta_4 \cdot G_{1i} + \beta_5 \cdot G_{2i})$$

\underline{Hipótesis}: las hipótesis del test se dividen en 2 casos:

- Caso 1): la $j-$ésima variable tiene un solo $\beta_j$ asociado a ella
...
- Caso 2): la $j-$ésima variable tiene más de un $\beta_j$ asociado a ella

\begin{align*}
\begin{cases}
H_0) &\, \beta_j = \beta_{j'} = 0 \\
&\qquad\qquad \qquad \qquad \qquad \text{con} \; (j, j') = \text{(4, 5) o (6, 7)} \\
...
El modelo que se obtendría sería $h_i(t) = h_0(t) \cdot \exp(\beta_1 \cdot \text{age}_i + \beta_2 \cdot G_{1i} + \beta_3 \cdot G_{2i} + \beta_4 \cdot S_{1i} + \beta_5 \cdot S_{2i})$. Sin embargo, este no es el modelo final dado que falta evaluar si ahora, con otras variables en el modelo, aquellas que no resultaron significativas de manera univariada lo son.
...
De esta manera, el modelo sería $h_i(t) = h_0(t) \cdot \exp(\beta_1 \cdot \text{age}_i + \beta_2 \cdot \text{chemo}_i + \beta_3 \cdot G_{1i} + \beta_4 \cdot G_{2i} + \beta_5 \cdot S_{1i} + \beta_6 \cdot S_{2i})$
...
$$
h_i(t) = h_0(t) \cdot \exp(\beta_1 \cdot \text{age}_i + \beta_2 \cdot \text{chemo}_i + \beta_3 \cdot G_{1i} + \beta_4 \cdot G_{2i} + \beta_5 \cdot S_{1i} + \beta_6 \cdot S_{2i})
$$
...
```{r}
# Modelos usando la mejor categorización para edad. Pruebo un modelo estratificando por chemo y otro sin estratificar para cada forma de incluir a la variable edad. En total son 6 modelos
...
cox.zph(modelo_estrat) # GLOBAL:0.03
...
# Edad numerica (modelo q teniamos)
modelo_estrat <- coxph(Surv(dtime, death) ~
                      age + chemo+
                      grade + size, ties = "breslow",
                      data = datos2)
...
modelo_estrat <- coxph(Surv(dtime, death) ~
                      ...
                      data = datos2)
# test2 <- cox.zph(modelo_estrat)
cox.zph(modelo_estrat) # GLOBAL:0.046
...
modelo_estrat <- coxph(Surv(dtime, death) ~
                      ...
                      data = datos2)
# test2 <- cox.zph(modelo_estrat)
cox.zph(modelo_estrat) # GLOBAL:0.01
```
...
```{r}
# Efectivamente el problema está en las mujeres mayores. Voy a tratar de encontrar cuál es la edad exacta en la que se dejan de cumplir los supuestos del modelo.

# --- Agrego un grupo más, uno entre 65 y 75 años ---

datos_prueba <- datos %>% 
  mutate(
    age_d1 = ifelse(age >= 40 & age < 50, 1, 0),
    age_d2 = ifelse(age >= 50 & age < 65, 1, 0),
    age_d3 = ifelse(age >= 65 & age < 75, 1, 0),
    age_d4 = ifelse(age >= 75, 1, 0)
  ) 

modelo_prueba <- coxph(Surv(dtime, death) ~ age_d1 + age_d2 +
                         age_d3 + age_d4 + chemo + grade +
                         size, ties = "breslow", 
                         data = datos_prueba)

# test2 <- cox.zph(modelo_prueba)
cox.zph(modelo_prueba)

# El problema está en las mujeres mayores de 75 años

# --- Agrego un grupo más, uno entre 75 y 85 años ---
...
datos_prueba <- datos %>% 
  mutate(
    ...
    age_d2 = ifelse(age >= 50 & age < 65, 1, 0),
    age_d3 = ifelse(age >= 65 & age < 75, 1, 0),
    age_d4 = ifelse(age >= 75 & age < 85, 1, 0),
    age_d5 = ifelse(age >= 85, 1, 0)
  ) 

modelo_prueba <- coxph(Surv(dtime, death) ~ age_d1 + age_d2 +
                         age_d3 + age_d4 + age_d5 + chemo +
                         grade + size, ties = "breslow", 
                         data = datos_prueba)
...
# Las dummies 4 y 5 no dan. 

# --- Vuelvo a probar con 4 dummies, pero hago la 3ra hasta 80 años ---

datos_prueba <- datos %>% 
  mutate(
    age_d1 = ifelse(age >= 40 & age < 50, 1, 0),
    age_d2 = ifelse(age >= 50 & age < 65, 1, 0),
    age_d3 = ifelse(age >= 65 & age < 80, 1, 0),
    age_d4 = ifelse(age >= 80, 1, 0)
  ) 
...
# test2 <- cox.zph(modelo_prueba)
cox.zph(modelo_prueba)
# Las dummies 3 y 4 dan mal.

# --- Hago la 3er dummie hasta 78 años ---

datos_prueba <- datos %>% 
  mutate(
    age_d1 = ifelse(age >= 40 & age < 50, 1, 0),
    age_d2 = ifelse(age >= 50 & age < 65, 1, 0),
    age_d3 = ifelse(age >= 65 & age < 78, 1, 0),
    age_d4 = ifelse(age >= 78, 1, 0)
  ) 
...
# test2 <- cox.zph(modelo_prueba)
cox.zph(modelo_prueba)
# EL TOPE DE EDAD PARA EL CUAL SE CUMPLE EL SUPUESTO ES 78 AÑOS

# Pruebo entonces de hacer un modelo con la edad continua, pero tomando sólo las mujeres hasta 78 años

# sum(datos$age>78) = 29, esa es la cantidad de datos que perdemos.

# Al final le pongo 65 años porque con menos no se cumple

datos_sin_viejas <- datos %>% 
  filter(age <= 65)

modelo_sin_viejas <- coxph(Surv(dtime, death) ~ age + chemo + grade +
                   size, ties = "breslow", 
                 data = datos_sin_viejas)

cox.zph(modelo_sin_viejas)
