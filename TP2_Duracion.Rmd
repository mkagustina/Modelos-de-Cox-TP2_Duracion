...
\vspace{-0.5cm}
\begin{center}
\footnotesize{\textit{Tabla 2: resultados de test de hipótesis para .....}}
\end{center}
...
\vspace{-0.5cm}
\begin{center}
\footnotesize{\textit{Tabla 3: resultados de test de hipótesis para .....}}
\end{center}
...
\vspace{-0.5cm}
\begin{center}
\footnotesize{\textit{Tabla 4: resultados de test de hipótesis para .....}}
\end{center}
...
\vspace{-0.5cm}
\begin{center}
\footnotesize{\textit{Tabla 5: resultados de test de hipótesis para .....}}
\end{center}

Si bien con un nivel de significación del 5% el modelo estratificado cumple el supuesto de hazards proporcionales (Tabla x), la probabilidad asociada al test sin estratificar es muy cercano a 0.05 y ese modelo brinda la posibilidad de estimar un parámetro para *Quimioterapia*, que es una variable importante para el problema. Por lo tanto, se decide  con ese modelo. De esta forma, el modelo final contiene las siguientes variables:
...
#### Grado y Tamaño: 
Es sabido que un mayor grado de diferenciciación más rápida es la dispersión del cancer, por lo que se calcula la razón de hazard para el grado de diferenciación 3 y comparando, por un lado, el tamaño del tumor menor a 20 mm contra el tamaño entre 20 y 50 mm; por otro, el tamaño del tumor menora 20 mm contra el tamaño mayor a 50 mm.

$$\circ \; \hat{RH}_{(<20 \; \text{vs} \; 20\text{-}50)} = \frac{\hat{h_0} (t) \cdot exp(-0.05 \cdot A_{1i} + 0.5 \cdot A_{2i} + 0.77 \cdot A_{3i} + 0.35 \cdot \text{chemo}_i + 0.31 \cdot 1 + 0.38 \cdot 1 + 0.84 \cdot 0)}{\hat{h_0} (t) \cdot exp(-0.05 \cdot A_{1i} + 0.5 \cdot A_{2i} + 0.77 \cdot A_{3i} + 0.35 \cdot \text{chemo}_i + 0.31 \cdot 1 + 0.38 \cdot 0 + 0.84 \cdot 0)} \Longrightarrow $$

$$ \Longrightarrow \hat{RH}_{(<20 \; \text{vs} \; 20\text{-}50)} = exp(0.38) = 1.46 $$
$$ \circ \; IC_{RH} = e^{[\hat{\beta}_6 \; \pm \; Z_{1-\frac{\alpha}{2}} \cdot \hat{SE}(\hat{\beta}_6) ]} = (e^{0.149};e^{0.611}) = (1.161; 1.842)$$


$$\circ \; \hat{RH}_{(<20 \; \text{vs} \; >50)} = \frac{\hat{h_0} (t) \cdot exp(-0.05 \cdot A_{1i} + 0.5 \cdot A_{2i} + 0.77 \cdot A_{3i} + 0.35 \cdot \text{chemo}_i + 0.31 \cdot 1 + 0.38 \cdot 0 + 0.84 \cdot 1)}{\hat{h_0} (t) \cdot exp(-0.05 \cdot A_{1i} + 0.5 \cdot A_{2i} + 0.77 \cdot A_{3i} + 0.35 \cdot \text{chemo}_i + 0.31 \cdot 1 + 0.38 \cdot 0 + 0.84 \cdot 0)} \Longrightarrow $$
$$ \Longrightarrow \hat{RH}_{(<20 \; \text{vs} \; >50)} = exp(0.84) = 2.32 $$

$$ \circ \; IC_{RH} = e^{[\hat{\beta}_7 \; \pm \; Z_{1-\frac{\alpha}{2}} \cdot \hat{SE}(\hat{\beta}_7) ]} = (e^{0.526};e^{1.154}) = (1.692; 3.171)$$

Por lo que el riesgo de tener cáncer de mama en pacientes con grado 3 de diferenciación y un tumor entre 20 y 50 mm es como minimo un 16.1% mayor y como máximo un 84.2% mayor que ese mismo riesgo para aquellas pacientes con grado 3 de diferenciación y un tumor de menos de 20 mm. Mientras que el riesgo de tener cáncer de mama para pacientes con grado 3 de diferenciación y un tumor de más de 50 mm es, como mínimo, un 69.2% mayor y, como máximo, un 217% mayor que ese mismo riesgo para aquellas pacientes también con grado 3 pero con un tumor de menos de 20mm.


#### Tamaño y Quimioterapia: 
