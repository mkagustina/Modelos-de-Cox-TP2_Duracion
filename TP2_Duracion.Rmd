...
library(tidyverse)
library(gridExtra)
library(survival)  # Para el ajuste del modelo de Cox 
...
-   Modelo aditivo: $ h_i (t) = h_0 (t) . exp( \beta_1 . edad_i + \beta_2 . meno_i + \beta_3 . er_i + \beta_4 . G_{1i} + \beta_5 . G_{2i} + \beta_6 . S_{1i} + \beta_7 . S_{2i}) $

-   Modelos de comparación:

    1)  Sin la variable *Tamaño*: $ h_i (t) = h_0 (t) . exp( \beta_1 . edad_i + \beta_2 . meno_i + \beta_3 . er_i + \beta_4 . G_{1i} + \beta_5 . G_{2i})$
    
    2) Sin la variable *Grado*: $ h_i (t) = h_0 (t) . exp( \beta_1 . edad_i + \beta_2 . meno_i + \beta_3 . er_i + \beta_6 . S_{1i} + \beta_7 . S_{2i})$
    
    3) Sin la variable *Receptores de estrógeno*: $ h_i (t) = h_0 (t) . exp( \beta_1 . edad_i + \beta_2 . meno_i + \beta_4 . G_{1i} + \beta_5 . G_{2i} + \beta_6 . S_{1i} + \beta_7 . S_{2i}) $
    
    4) Sin la variable *Menopausía*: $ h_i (t) = h_0 (t) . exp( \beta_1 . edad_i + \beta_3 . er_i + \beta_4 . G_{1i} + \beta_5 . G_{2i} + \beta_6 . S_{1i} + \beta_7 . S_{2i}) $
    
    5) Sin la variable *Edad*: $ h_i (t) = h_0 (t) . exp( \beta_2 . meno_i + \beta_3 . er_i + \beta_4 . G_{1i} + \beta_5 . G_{2i} + \beta_6 . S_{1i} + \beta_7 . S_{2i}) $
...
El modelo que se obtendría sería $ h_i (t) = h_0 (t) . exp( \beta_1 . edad_i + \beta_2 . G_{1i} + \beta_3 . G_{2i} + \beta_4 . S_{1i} + \beta_5 . S_{2i} ) $. Sin embargo, este no es el modelo final dado que falta evaluar si, ahora con otras variables en el modelo, aquellas que no resultaron significativas de manera singular (NOTA PARA AGUS: sé que guille usó una palabra para ese tipo de ajustes pero no puedo recordar cuál es; lo hizo en el ejercicio de seleccion de variables cuando hay una de interés principal -tratamiento-).
...
lrtest(mod_edad_pgrf, mod_edad_pgr)
```
